...
class MariadbIAMAuthExecutorTest(
    @Autowired val JDBCExecutorService: JDBCExecutor,
    @Value("\${aws.db.mariadbhost}") private val awsDbHost: String,
) {
    val executionRequestId = ExecutionRequestId("5Wb9WJxCxej5W1Rt6cTBV6")
    ...
    fun `test IAM authentication and simple query`() {
        val connection = DatasourceConnection(
            id = ConnectionId("aws-rds-mariadb-001"),
            displayName = "AWS RDS MariaDB Database",
            description = "MariaDB database hosted on AWS RDS",
            ...
            protocol = DatabaseProtocol.MARIADB,
            additionalOptions = "",
            dumpsEnabled = false,
