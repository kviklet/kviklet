...
class ProxyInstance(val port: Int, val connectionString: String, val proxy: PostgresProxy, val connection: Connection, val eventService: EventServiceMock) {
...
fun directConnectionFactory(postgresContainer: PostgreSQLContainer<Nothing>) : Connection {
    return DriverManager.getConnection(postgresContainer.jdbcUrl, getPostgresContainerConnProps())
}

fun proxyServerFactory(postgresContainer: PostgreSQLContainer<Nothing>, executionRequestAdapter: ExecutionRequestAdapter, eventAdapter: EventAdapter, tlsCertificate: TLSCertificate? = null) : ProxyInstance {
